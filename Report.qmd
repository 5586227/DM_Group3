...
In summary, four new entities were created from the normalization, which were ORDER_DETAIL, ORDER_ITEM, DISCOUNT, and ADDRESS.

-   CUSTOMER([customer_id]{.ul}, first_name, last_name, gender, email, mobile_no, address_id)

-   ADDRESS([address_id]{.ul}, postcode, street, city, country)

-   DISCOUNT([promo_code]{.ul}, discount_amount)

<<<<<<< HEAD
-   ADVERTISEMENT([ad_id]{.underline}, ad_frequency, ad_place, ad_price) 
=======
-   ADVERTISEMENT([ad_id]{.ul}, ad_prequency, ad_place, ad_price) 
>>>>>>> c5f42cbd9c0402d3938ce9d65c3337605310b142

-   SUPPLIER([supplier_id]{.ul}, supplier_name, email, mobile_no) 

-   CATEGORY([category_id,]{.ul} category_name, category_fee)

-   PRODUCT([product_id]{.ul}, product_name, product_description, product_rating, unit_price, stock_at_hand, main_product_id, category_id, supplier_id) 

-   ORDER_DETAIL([order_id,]{.ul} customer_id, order_date, order_status, promo_code, payment_method, delivery_fee)

-   ORDER_ITEM([order_id]{.ul}, product_id, quantity)

-   ADVERTISE_IN([product_id]{.ul}, ad_id)
