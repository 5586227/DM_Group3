...
The first adjustment was on the ORDER table. After reviewing the order of normal forms, it was found that the ORDER entity still did not comply with 3NF. **\<will continue explain later\>**
...
This indicated that street, city, and country were transitively dependent on postcode. As a result, a new table is created as ADDRESS, which has postcode as a primary key and store street, city, and country as other attributes
...
The imported data sets were validated before being added to the database to ensure that they aligned with the nature of the keys and data type specified in the physical schema.

We would first start by validating parent entities
...
Once all parent entities were validated, we would continue validating child entities
