...
for (table_name in names(data_frames)) {
  if (table_name %in% names(primary_keys)) {
    ...
    if (any(missing_values)) {
      empty_row_indices <- apply(data_frames[[table_name]][,
                                                           -which(names(data_frames[[table_name]]) %in% columns_to_skip)], 1, function(row) 
                                                             any(row == "" |
                                                                                                                                                  is.na(row)))
      data_frames[[table_name]] <- data_frames[[table_name]][
        !empty_row_indices, ]
    }
  }
}
