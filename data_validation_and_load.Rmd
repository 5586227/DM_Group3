...
#Check for missing data (remove?)
na_order_item_order_id <- order[is.na(order$order_id), "order_id"]
na_order_item_product_id <- order[is.na(order$product_id), c("order_id", "product_id")]
na_order_item_order_quantity <- order[is.na(order$order_quantity), c("order_id", "order_quantity")]

#Remove unclean data
bad_order_item_record <- unique(c(duplicate_order_id$order_id,
...
invalid_order_fk <- order_detail[!order_detail$order_id %in% order_item$order_id, "order_id"]
...
#Check if total order quantity for a product in all orders exceed stock on hand
